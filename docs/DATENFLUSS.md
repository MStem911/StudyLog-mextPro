...
| **VR-Szenario-Durchläufe im Ablauf** (Teil von `sl_probanden`, `p.szenarien`) | Objekt `{ [stepId]: [ { id, label (Freitext), phases } ] }` je Schritt 7 (Tutorial), 8 (Hologate) und 12 (Rollercoaster). `phases` = Teilschritte je Durchlauf; „starten"/„beendet" speichern einen ISO-8601-Zeitstempel (Gerätezeit) beim Antippen, die übrigen Phasen sind reine Häkchen. **Schritt 7:** frei anlegbare Durchläufe, Phasen Tutorial starten/kalibriert/durchläuft/beendet/„direkt gewechselt". **Schritt 8:** genau **5 feste Durchläufe** (Scheiben, Köpfe, Laufen, Drohnen, Kombi — nicht änderbar), je nur „Szenario starten" + „Szenario beendet". **Schritt 12:** frei anlegbare Durchläufe, Phasen starten/kalibriert/durchläuft/beendet/Brille abgezogen/Selbstbewertung+Bewertungsbogen. Personenbezug über den pseudonymisierten Datensatz | Zeitliche Feingliederung des VR-Szenario-Ablaufs je Durchlauf | TODO: Datenschutz prüfen | `localStorage`, lokal | Unbegrenzt, bis manuelle Löschung (Durchlauf einzeln [Schritt 7/12], Person löschen oder "Alle Daten löschen") | Jeweilige Studienleitung / Gerätebesitzer:in |
