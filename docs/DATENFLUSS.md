...
- **CSV/JSON-Export:** Die im Ablauf erfassten Daten (`p.ablauf`, `p.bewertungen`,
  `p.ereignisse`, `p.sensorik`, `p.sensorikAblegen`, `p.szenarien`) sind aktuell **nicht** Teil
  des Exports — der Export basiert noch auf den alten `sl_sessions`/`sl_bewertungen`. Sie
  verbleiben ausschließlich in
