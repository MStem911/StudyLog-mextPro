...
flowchart LR
    subgraph Input["Erfassung (UI-Eingabe durch Studienleitung)"]
        ...
        I9["Ablauf-Zeitleiste (Bereich 'Ablauf') je Teilnehmende:r:<br/>pro Studienschritt (feste Liste, Schritte 1–16)<br/>Start-/Endzeit (Gerätezeit, Button 'Jetzt' oder<br/>manuelle Eingabe) + Freitext-Anmerkung —<br/>Teil von sl_probanden (p.ablauf)"]
    end
...
| **VR-Szenario-Durchläufe im Ablauf** (Teil von `sl_probanden`, `p.szenarien`) | Objekt `{ [stepId]: [ { id, label (Freitext), phases } ] }` je Schritt 6 (Tutorial), 7 (Hologate) und 10 (Rollercoaster). `phases` = Teilschritte je Durchlauf; „starten"/„beendet" speichern einen ISO-8601-Zeitstempel (Gerätezeit) beim Antippen, die übrigen Phasen sind reine Häkchen bzw. (Schritt 6) nur Anzeige. **Schritt 6:** genau **ein** fester Durchlauf; erfasst werden nur „Tutorial starten" und „Tutorial beendet" (Zeitstempel), die Punkte „Person kalibriert" / „Person durchläuft das Tutorial" / „Direkt ins VR-Szenario gewechselt" sind nicht abhakbar (reine Ablauf-Erinnerung, kein Datenfeld). **Schritt 7:** genau **5 feste Durchläufe** (Scheiben, Köpfe, Laufen, Drohnen, Kombi — nicht änderbar), je nur „Szenario starten" + „Szenario beendet". **Schritt 10:** frei anlegbare Durchläufe, Phasen starten/kalibriert/durchläuft/beendet/Brille abgezogen/Selbstbewertung+Bewertungsbogen. Personenbezug über den pseudonymisierten Datensatz | Zeitliche Feingliederung des VR-Szenario-Ablaufs je Durchlauf | TODO: Datenschutz prüfen | `localStorage`, lokal | Unbegrenzt, bis manuelle Löschung (Durchlauf einzeln [Schritt 10], Person löschen oder "Alle Daten löschen") | Jeweilige Studienleitung / Gerätebesitzer:in |
