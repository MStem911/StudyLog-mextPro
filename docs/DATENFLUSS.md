...
| **VR-Szenario-Durchläufe im Ablauf** (Teil von `sl_probanden`, `p.szenarien`) | Objekt `{ [stepId]: [ { id, label (Freitext), phases } ] }` je Schritt 7 (Hologate) und 10 (Rollercoaster). `phases` = Teilschritte je Durchlauf; „starten"/„beendet" speichern einen ISO-8601-Zeitstempel (Gerätezeit). **Seit v2.29.0** werden diese beiden Zeit-Phasen über ein normales Zeit-Eingabefeld erfasst/korrigiert (Button „Jetzt" oder manuelle Eingabe, wie bei den übrigen Schritt-Zeitfeldern) statt per einfachem Antippen; die übrigen Phasen bleiben reine Häkchen (antippen, Rückfrage beim Entfernen). **Schritt 7:** genau **5 feste Durchläufe** (Scheiben, Köpfe, Laufen, Drohnen, Kombi — nicht änderbar), je nur „Szenario starten" + „Szenario beendet". **Schritt 10:** frei anlegbare Durchläufe, Phasen starten/kalibriert/durchläuft/beendet/Brille abgezogen/Selbstbewertung+Bewertungsbogen. Personenbezug über den pseudonymisierten Datensatz. **Schritt 6 (Tutorial) ist seit v2.28.0 nicht mehr Teil dieses Datentyps** — bereits erfasste Zeitstempel wurden beim Laden einmalig nach `p.ablauf.fs_06.startISO`/`endISO` migriert (siehe oben) | Zeitliche Feingliederung des VR-Szenario-Ablaufs je Durchlauf | TODO: Datenschutz prüfen | `localStorage`, lokal | Unbegrenzt, bis manuelle Löschung (Durchlauf einzeln [Schritt 10], Person löschen oder "Alle Daten löschen") | Jeweilige Studienleitung / Gerätebesitzer:in |
