...
flowchart LR
    subgraph Input["Erfassung (UI-Eingabe durch Studienleitung)"]
        ...
        I9["Ablauf-Zeitleiste (Bereich 'Ablauf') je Teilnehmende:r:<br/>pro Studienschritt (feste Liste, Schritte 1–16)<br/>Start-/Endzeit (Gerätezeit, Button 'Jetzt' oder<br/>manuelle Eingabe) —<br/>Teil von sl_probanden (p.ablauf)"]
    end
...
- **"Schritt leeren" im Bereich Ablauf:** Nur an Schritten mit Zeitfeldern verfügbar. Entfernt
  Start-/Endzeit des betreffenden Schritts aus `p.ablauf` der aktiven Person (nach
  ...
  Andere Schritte und Personen bleiben unverändert.
