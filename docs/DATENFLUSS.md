...
| **VR-Szenario-Durchläufe im Ablauf** (Teil von `sl_probanden`, `p.szenarien`) | Objekt `{ [stepId]: [ { id, label (Freitext), phases } ] }` je Schritt 7 (Tutorial), 8 (5 Hologate-Durchläufe) und 12 (Rollercoaster). `phases` = feste Teilschritte je Durchlauf: „Szenario starten" und „Szenario beendet" speichern einen ISO-8601-Zeitstempel (Gerätezeit) beim Abhaken, „Person kalibriert" / „Person durchläuft das Szenario" / „Brille abgezogen" / „Selbstbewertung + Bewertungsbogen" sind reine Häkchen. Personenbezug über den pseudonymisierten Datensatz | Zeitliche Feingliederung des VR-Szenario-Ablaufs je Durchlauf | TODO: Datenschutz prüfen | `localStorage`, lokal | Unbegrenzt, bis manuelle Löschung (Durchlauf einzeln, Person löschen oder "Alle Daten löschen") | Jeweilige Studienleitung / Gerätebesitzer:in |
